...
import {
  ...
  Plane, CheckCircle2, ArrowLeft, Shield, Loader2
} from "lucide-react";
import { Label } from "@/components/ui/label";
import { Checkbox } from "@/components/ui/checkbox";
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select";

// ✅ Import unique — plus de redéfinition locale
...
// ✅ REGIONS déclaré ici, au niveau module — plus jamais à l'intérieur d'un hook
const REGIONS = [
...
// ─────────────────────────────────────────────────────────────
// COMPOSANT : Carte 3D Côte d'Ivoire
// ─────────────────────────────────────────────────────────────
function IvoryCoast3D() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < N; i++) {
      pp[i*3] = (Math.random()-.5)*90; pp[i*3+1] = (Math.random()-.5)*70; pp[i*3+2] = (Math.random()-.5)*45;
      const c = pal[i % 4]; pc[i*3] = c.r; pc[i*3+1] = c.g; pc[i*3+2] = c.b;
    }
    ...
    window.addEventListener("mousedown", onMD); window.addEventListener("mouseup", onMU);
    window.addEventListener("mousemove", onMM); window.addEventListener("resize", onRz);

    let raf;
    const tick = () => {
      raf = requestAnimationFrame(tick);
      if (auto) { tgt.y += .004; tgt.x = .3 + Math.sin(Date.now() * .0005) * .18; }
      cur.x += (tgt.x - cur.x) * .055; cur.y += (tgt.y - cur.y) * .055;
      mapGroup.rotation.x = cur.x; mapGroup.rotation.y = cur.y;
    ...
    return () => {
      ...
      window.removeEventListener("mousedown", onMD); window.removeEventListener("mouseup", onMU);
      window.removeEventListener("mousemove", onMM); window.removeEventListener("resize", onRz);
      renderer.dispose();
...
function StrengthBar({ pwd }) {
  ...
  const labs = ["Faible", "Moyen", "Bon", "Fort ✦"];
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-1.5 items-center mt-1.5">
      {[0, 1, 2, 3].map(i => (
        <motion.div key={i} className="h-0.5 flex-1 rounded-full"
          style={{ background: i < s ? cols[s - 1] : "rgba(255,255,255,0.08)" }}
          initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ delay: i * .05 }} />
      ))}
...
const BASE_STYLE = {
  ...
  border: "1px solid rgba(200,151,58,0.3)", color: "#F5E6C8",
...
export default function RegisterPage() {
  ...
  // ✅ Hook Firebase — un seul, celui importé en haut
  const { register, loading: submitting, error: firebaseError, resetError } = useRegister();
  ...
  const validate1 = () => {
    const e = {};
    if (!form.pseudo.trim() || form.pseudo.length < 3) e.pseudo = "Pseudo requis (min. 3 car.)";
  ...
  const validate2 = () => {
    const e = {};
    if (isIvorian === null)                    e.ivorian = "Choisir une option";
    if (isIvorian === true && !form.ethnie)    e.ethnie  = "Choisir votre ethnie";
    if (isIvorian === false && !form.profil)   e.profil  = "Choisir votre profil";
    if (!form.terms)                           e.terms   = "Accepter les conditions";
    setErrors(e);
    return Object.keys(e).length === 0;
  };

  // ── Soumission finale → Firebase ─────────────────────────
  const handleSubmit = async () => {
    ...
    if (result.success) {
      setDone(true); // → affiche l'écran de bienvenue
      // rediriger vers l'app après 3s
      setTimeout(() => {
        navigate("/app");
      }, 3000);
    } else {
      // L'erreur Firebase (ex: email déjà pris, pseudo pris) remonte ici
    ...
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden" style={{ background: "#0A0602", cursor: "none" }}>
      ...
      <div className="relative z-10 text-center flex flex-col items-center gap-8 px-8">
        <motion.div
          ...
          className="w-28 h-28 rounded-full border-2 border-amber-400 flex items-center justify-center"
          style={{ boxShadow: "0 0 80px rgba(200,151,58,0.6)" }}
        >
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .4 }}>
          ...
          <h1 className="text-5xl font-black text-amber-50 leading-tight" style={{ fontFamily: "'Playfair Display',serif" }}>
            Bienvenue,<br /><span className="italic text-amber-400">{form.pseudo}</span>
          </h1>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .9 }}>
          <button
            ...
            className="px-12 py-4 text-xs tracking-widest uppercase font-bold rounded-sm"
            style={{ background: "linear-gradient(135deg,#C8973A,#E8C060)", color: "#1A0E05", boxShadow: "0 12px 40px rgba(200,151,58,0.45)", fontFamily: "'Cinzel Decorative',serif" }}
          >
          ...
          <motion.div className="flex items-center gap-3 mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .25 }}>
            {[1, 2].map(n => (
              <div key={n} className="flex items-center gap-3 flex-1">
                ...
                {n < 2 && (
                  <div className="flex-1 h-px overflow-hidden rounded-full" style={{ background: "rgba(200,151,58,0.1)" }}>
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 1 && (
                  <motion.div key="s1" variants={pageVariants} initial="enter" animate="center" exit="exit" transition={{ duration: .4, ease: [.25, .8, .25, 1] }} className="space-y-4">
                    <Field label="Pseudo" icon={User} error={errors.pseudo} delay={0}>
                      <input style={BASE_STYLE} placeholder="nom_ancestral" value={form.pseudo}
                        onChange={e => { setField("pseudo", e.target.value); clrErr("pseudo"); }}
                        onFocus={handleFocus} onBlur={handleBlur} />
                    </Field>
                    <Field label="Email — ton sceau" icon={Mail} error={errors.email} delay={.08}>
                      <input style={BASE_STYLE} type="email" placeholder="toi@exemple.ci" value={form.email}
                        onChange={e => { setField("email", e.target.value); clrErr("email"); }}
                        onFocus={handleFocus} onBlur={handleBlur} />
                    </Field>
                    <Field label="Mot de passe sacré" icon={Lock} error={errors.password} delay={.16}>
                      <div className="relative">
                        <input style={{ ...BASE_STYLE, paddingRight: "3rem" }} type={showPass ? "text" : "password"} placeholder="••••••••" value={form.password}
                          onChange={e => { setField("password", e.target.value); clrErr("password"); }}
                          onFocus={handleFocus} onBlur={handleBlur} />
                    ...
                    <Field label="Confirmer le sceau" icon={Shield} error={errors.confirm} delay={.24}>
                      <div className="relative">
                        <input style={{ ...BASE_STYLE, paddingRight: "3.5rem" }} type={showConf ? "text" : "password"} placeholder="••••••••" value={form.confirm}
                          onChange={e => { setField("confirm", e.target.value); clrErr("confirm"); }}
                          onFocus={handleFocus} onBlur={handleBlur} />
                ...
                {step === 2 && (
                  <motion.div key="s2" variants={pageVariants} initial="enter" animate="center" exit="exit" transition={{ duration: .4, ease: [.25, .8, .25, 1] }} className="space-y-5">
                    ...
                    {/* CGU — ✅ Checkbox décommentée */}
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .3 }} className="flex items-start gap-3 pt-1">
                      <Checkbox
                        id="terms"
                        checked={form.terms}
                        onCheckedChange={v => { setField("terms", v); clrErr("terms"); }}
                        className="border-amber-500/30 data-[state=checked]:bg-amber-500 data-[state=checked]:border-amber-500 mt-0.5"
                      />
                    ...
                    {/* ✅ Erreur globale Firebase (pseudo pris, email pris, réseau…) */}
                    <AnimatePresence>
                      {(errors.global || firebaseError) && (
                        <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                          className="text-red-400/90 text-xs text-center px-3 py-2 rounded border border-red-500/20"
                          style={{ background: "rgba(239,68,68,0.08)" }}>
                          {errors.global || firebaseError?.message}
                        </motion.div>
                      )}
                    </AnimatePresence>
              ...
              <div className="flex gap-3 w-full">
                {step === 2 && (
                  <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }}>
                    <button type="button" onClick={() => { setStep(1); setErrors({}); resetError(); }}
                      disabled={submitting}
                      className="h-full px-4 rounded border transition-all hover:border-amber-500/40"
                      style={{ borderColor: "rgba(200,151,58,0.2)", color: "rgba(245,230,200,0.4)", background: "rgba(0,0,0,0.2)" }}>
                ...
                <motion.button
                  ...
                  style={{ background: "linear-gradient(135deg,#C8973A 0%,#E8C060 50%,#C8973A 100%)", backgroundSize: "200%", color: "#1A0E05", fontFamily: "'Cinzel Decorative',serif", fontSize: ".68rem", letterSpacing: ".2em", fontWeight: "700", boxShadow: "0 10px 36px rgba(200,151,58,0.42),0 2px 8px rgba(200,151,58,0.2)", opacity: submitting ? 0.7 : 1 }}
                  whileHover={!submitting ? { scale: 1.025, boxShadow: "0 14px 48px rgba(200,151,58,0.6)" } : {}}
                  whileTap={!submitting ? { scale: .975 } : {}}
                  disabled={submitting}
                  onClick={() => {
                    if (step === 1) { if (validate1()) setStep(2); }
                    else            { handleSubmit(); }         // ✅ appel Firebase réel
                  }}
                >
                  <span className="relative z-10 flex items-center justify-center gap-2">
                    {submitting ? (
                      // ✅ Spinner pendant la requête Firebase
          ...
          <div className="flex justify-center gap-2 mt-5">
            {[0, 1, 2, 3, 4].map(i => (
              <motion.div key={i} className="h-px rounded-full"
                ...
                animate={{ width: 24, opacity: i < (step === 1 ? 2 : 5) ? 0.5 : 0.1 }}
                transition={{ delay: .9 + i * .07 }}
                style={{ background: "linear-gradient(90deg,#C8973A,#E8C060)" }} />
            ))}
          </div>
        </div>
      </div>
