...
import {
  ...
  TriangleAlert, BadgeCheck, LockKeyhole, Sparkles
} from "lucide-react";
...
function IvoryCoast3D() {
  ...
  useEffect(() => {
    ...
    const N = 500; const pp = new Float32Array(N * 3); const pc = new Float32Array(N * 3);
...
const BASE_STYLE = {
  width:"100%",background:"rgba(0,0,0,0.28)",border:"1px solid rgba(200,151,58,0.3)",
  color:"#F5E6C8",fontFamily:"'Cormorant Garamond',serif",fontSize:"1.05rem",
  letterSpacing:"0.02em",borderRadius:"4px",padding:".85rem 1rem",outline:"none",
...
export default function LoginPage() {
  ...
  return (
    <div className="relative min-h-screen flex overflow-hidden" style={{background:"#0A0602",cursor:"none"}}>
      ...
      {/* Dégradé vers droite pour faire respirer la carte */}
          ...
          <div className="glass-card rounded-md overflow-hidden">
            ...
            <div className="px-7 pb-2">
              <form onSubmit={onSubmit} className="space-y-4">
                <Field label="Email — ton sceau" icon={Mail} error={errors.email} delay={0}>
                  <input style={BASE_STYLE} type="email" placeholder="toi@exemple.ci" value={form.email}
                    onChange={e=>{setField("email",e.target.value);clrErr("email");}} onFocus={handleFocus} onBlur={handleBlur}/>
                </Field>
                ...
                <Field label="Mot de passe sacré" icon={Lock} error={errors.password} delay={.08}>
                  <div className="relative">
                    <input style={{...BASE_STYLE,paddingRight:"3rem"}} type={showPass?"text":"password"} placeholder="••••••••" value={form.password}
                      onChange={e=>{setField("password",e.target.value);clrErr("password");}} onFocus={handleFocus} onBlur={handleBlur}/>
                ...
                <AnimatePresence>
                  {status==="error" && (
                    <motion.div initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0}} className="flex items-center gap-2 px-3 py-2 rounded text-sm text-red-300" style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.25)"}}>
                      <TriangleAlert className="w-4 h-4 flex-shrink-0"/><span>Identifiants incorrects.</span>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="pt-3 pb-2 flex flex-col gap-4">
                  <motion.button type="submit" disabled={status==="loading"}
                    ...
                    style={{background:"linear-gradient(135deg,#C8973A 0%,#E8C060 50%,#C8973A 100%)",backgroundSize:"200%",color:"#1A0E05",fontFamily:"'Cinzel Decorative',serif",fontSize:".68rem",letterSpacing:".2em",fontWeight:"700",boxShadow:"0 10px 36px rgba(200,151,58,0.42)",opacity:status==="loading"?.7:1}}
                    whileHover={{scale:1.025,boxShadow:"0 14px 48px rgba(200,151,58,0.6)"}} whileTap={{scale:.975}}
                  >
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {status==="loading"?"Invocation en cours…":"Réouvrir le portail"}
                      {status!=="loading" && <motion.span animate={{x:[0,4,0]}} transition={{repeat:Infinity,duration:1.6}}><ChevronRight className="w-4 h-4"/></motion.span>}
                    </span>
                  </motion.button>

                  <p className="text-center text-sm text-amber-100/25" style={{fontFamily:"'Cormorant Garamond',serif"}}>
                    Pas encore de sanctuaire ?{" "}
                    <Link to="/auth/register" className="text-amber-400/70 hover:text-amber-300 transition-colors border-b border-amber-400/20 pb-px">
                      Créer mon compte
                    </Link>
                  </p>
                </div>
