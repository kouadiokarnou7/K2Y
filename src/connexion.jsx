...
import {
  ...
  ArrowLeft, CheckCircle2, TriangleAlert,
  BadgeCheck, Loader2, User
} from "lucide-react";
...
function IvoryCoast3D(){
  ...
  useEffect(()=>{
    ...
    REGIONS.forEach(r=>{try{
      ...
      const geo=new THREE.ExtrudeGeometry(shape,{depth:1.4,bevelEnabled:true,bevelThickness:.18,bevelSize:.12,bevelSegments:3});
      ...
      const mesh=new THREE.Mesh(geo,[
        ...
        new THREE.MeshPhongMaterial({color:new THREE.Color(r.color).multiplyScalar(0.5),transparent:true,opacity:.95,shininess:40}),
      ]);
      mesh.position.z=-.7;mapGroup.add(mesh);
      const edges=new THREE.LineSegments(new THREE.EdgesGeometry(geo,22),new THREE.LineBasicMaterial({color:0xC8973A,transparent:true,opacity:.6}));
      edges.position.z=-.7;mapGroup.add(edges);
    }catch(_){}});
...
export default function LoginPage(){
  ...
  // ✅ useLogin branché sur Firebase — remplace le setTimeout fake
  const { login, loading, error: firebaseError, resetError } = useLogin();
  ...
  // ✅ Validation : accepte email OU pseudo (min 3 car.)
  const validate=()=>{
    const e={};
    const id=form.identifier.trim();
    if(!id||id.length<3) e.identifier="Email ou pseudo requis (min. 3 car.)";
    if(!form.password||form.password.length<6) e.password="Minimum 6 caractères";
    setErrors(e);
    return Object.keys(e).length===0;
  };

  // ✅ Soumission réelle vers Firebase
  const onSubmit=async(ev)=>{
    ...
    if(result.success){
      ...
      setErrors(e=>({...e,global:result.message}));
    }
  };
  ...
  if(success) return(
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden" style={{background:"#0A0602",cursor:"none"}}>
      ...
      <div className="relative z-10 text-center flex flex-col items-center gap-8 px-8">
        <motion.div initial={{scale:0,rotate:-180}} animate={{scale:1,rotate:0}} transition={{type:"spring",stiffness:200,damping:14}}
          className="w-24 h-24 rounded-full border-2 border-amber-400 flex items-center justify-center"
          style={{boxShadow:"0 0 60px rgba(200,151,58,0.6)"}}>
          <BadgeCheck className="text-amber-400 w-12 h-12"/>
        </motion.div>
        <motion.div initial={{opacity:0,y:30}} animate={{opacity:1,y:0}} transition={{delay:.4}}>
          <p className="text-amber-400/50 text-xs tracking-widest uppercase mb-3" style={{fontFamily:"'Cinzel Decorative',serif"}}>— Portail réouvert —</p>
          <h1 className="text-4xl font-black text-amber-50" style={{fontFamily:"'Playfair Display',serif"}}>Bienvenue<br/><span className="italic text-amber-400">de retour</span></h1>
        </motion.div>
      </div>
    </div>
  );
  ...
  return(
    <div className="relative min-h-screen flex overflow-hidden" style={{background:"#0A0602",cursor:"none"}}>
      ...
      <IvoryCoast3D/><MagicCursor/>

      <div className="absolute inset-0 z-[1] pointer-events-none" style={{background:"linear-gradient(to right,rgba(10,6,2,0.0) 0%,rgba(10,6,2,0.0) 48%,rgba(10,6,2,0.65) 100%)"}}/>
      ...
      {/* AKWABA gauche */}
      <div className="absolute left-0 top-0 bottom-0 z-[3] hidden lg:flex flex-col items-center justify-center pointer-events-none" style={{width:"42%",paddingLeft:"4rem"}}>
        ...
        <motion.p className="mt-8 text-amber-400/45 text-xs tracking-widest text-center leading-relaxed" style={{fontFamily:"'Cinzel Decorative',serif",maxWidth:160}} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:1.9}}>Bienvenue en<br/>Côte d'Ivoire</motion.p>
      ...
      <div className="relative z-10 flex items-center w-full justify-end">
        <div className="w-full lg:w-[460px] px-4 py-10 overflow-y-auto max-h-screen">
          ...
          <motion.div initial={{opacity:0,x:15}} animate={{opacity:1,x:0}} transition={{delay:.15}} className="mb-5">
            <Link to="/"><button className="flex items-center gap-2 text-amber-400/50 hover:text-amber-400 transition-colors" style={{fontFamily:"'Cormorant Garamond',serif",fontSize:".9rem",letterSpacing:".05em"}}><ArrowLeft className="w-4 h-4"/><span>Accueil</span></button></Link>
          </motion.div>

          <div className="glass-card rounded-md overflow-hidden">
            <div className="pt-7 px-7 pb-5">
              <motion.div initial={{opacity:0,y:12}} animate={{opacity:1,y:0}} transition={{duration:.36}}>
                ...
                <h2 className="text-3xl font-black leading-tight text-amber-50" style={{fontFamily:"'Playfair Display',serif"}}>
                  Réouvrir mon<br/><span className="italic text-amber-400">sanctuaire</span>
                </h2>
                <p className="text-amber-100/40 mt-2 text-sm leading-relaxed">Accède à tes mémoires et à l'héritage vivant.</p>
              </motion.div>
            </div>
            ...
            <div className="px-7 pb-2">
              <form onSubmit={onSubmit} className="space-y-4">
                ...
                {/* ✅ Champ identifiant : email OU pseudo */}
                <Field label="Email ou pseudo" icon={User} error={errors.identifier} delay={0}>
                  <input
                    style={BASE_STYLE}
                    type="text"
                    placeholder="toi@exemple.ci  ou  nom_ancestral"
                    value={form.identifier}
                    onChange={e=>{setField("identifier",e.target.value);clrErr("identifier");resetError();}}
                    onFocus={handleFocus} onBlur={handleBlur}
                    autoComplete="username"
                  />
                </Field>

                <Field label="Mot de passe sacré" icon={Lock} error={errors.password} delay={.08}>
                  <div className="relative">
                    <input style={{...BASE_STYLE,paddingRight:"3rem"}} type={showPass?"text":"password"} placeholder="••••••••" value={form.password}
                      onChange={e=>{setField("password",e.target.value);clrErr("password");resetError();}}
                ...
                <div className="flex justify-end">
                  <a href="#" className="text-amber-400/50 hover:text-amber-400 text-xs transition-colors border-b border-amber-400/20 pb-px" style={{fontFamily:"'Cormorant Garamond',serif",letterSpacing:".05em"}}>
                    Mot de passe oublié ?
                  </a>
                </div>

                {/* ✅ Erreur Firebase (identifiants incorrects, pseudo introuvable, réseau…) */}
                <AnimatePresence>
                  {(errors.global||firebaseError)&&(
                    <motion.div initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0}}
                      className="flex items-center gap-2 px-3 py-2 rounded text-sm text-red-300"
                      style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.25)"}}>
                      <TriangleAlert className="w-4 h-4 flex-shrink-0"/>
                      <span>{errors.global||firebaseError?.message}</span>
                    </motion.div>
                  )}
                </AnimatePresence>
                ...
                <div className="pt-3 pb-2 flex flex-col gap-4">
                  <motion.button type="submit" disabled={loading}
                    className="w-full py-4 text-center rounded relative overflow-hidden group"
                    style={{background:"linear-gradient(135deg,#C8973A 0%,#E8C060 50%,#C8973A 100%)",backgroundSize:"200%",color:"#1A0E05",fontFamily:"'Cinzel Decorative',serif",fontSize:".68rem",letterSpacing:".2em",fontWeight:"700",boxShadow:"0 10px 36px rgba(200,151,58,0.42)",opacity:loading?.7:1}}
                    whileHover={!loading?{scale:1.025,boxShadow:"0 14px 48px rgba(200,151,58,0.6)"}:{}}
                    whileTap={!loading?{scale:.975}:{}}
                  >
                    <span className="relative z-10 flex items-center justify-center gap-2">
                      {loading
                        ?<><Loader2 className="w-4 h-4 animate-spin"/>Invocation en cours…</>
                        :<>Réouvrir le portail<motion.span animate={{x:[0,4,0]}} transition={{repeat:Infinity,duration:1.6}}><ChevronRight className="w-4 h-4"/></motion.span></>
                      }
                    </span>
                  ...
                  <p className="text-center text-sm text-amber-100/25" style={{fontFamily:"'Cormorant Garamond',serif"}}>
                    ...
                    <Link to="/auth/register" className="text-amber-400/70 hover:text-amber-300 transition-colors border-b border-amber-400/20 pb-px">Créer mon compte</Link>
                  </p>
                </div>
              </form>
            </div>
          ...
          <div className="flex justify-center gap-2 mt-5">
            {[0,1,2].map(i=>(
              <motion.div key={i} className="h-px rounded-full" initial={{width:0,opacity:0}} animate={{width:24,opacity:.35}} transition={{delay:.9+i*.07}} style={{background:"linear-gradient(90deg,#C8973A,#E8C060)"}}/>
            ))}
          </div>
        </div>
      </div>
